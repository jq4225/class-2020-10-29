...
Probably more conservative based on Tuesday? Depends on demographics and other stuff we can factor in?
...
Formula: $attchg_i = \beta_1 * treatment + \epsilon_i$

Preceptor table: Row per person, Two columns indicating attitude w/ and w/o treatment.

We can't construct the ideal preceptor table with our data, but we're able to estimate average treatment effects on ppl who were and weren't treated. We can get averages for what people w/ similar demographics as Preceptor would think.
...
fit_treat %>%
  pivot_longer(cols = c("treatmentTreated", "treatmentControl"), names_to = "treatment",
               values_to = "change") %>%
  filter(treatment == "treatmentTreated") %>%
  ggplot(aes(x = change)) +  geom_histogram(alpha = 0.5,
                                                              bins = 50, color = "white")
